...
class PixiAtlas extends PIXI.AnimatedSprite{
	...
	constructor(sheet,parent,_id){

			let atlas = null;

			if(_id==undefined){
				atlas = parent.assets.loadSpritesheet(sheet).animations[sheet];
			}else{
				atlas = parent.assets.loadSpritesheet(sheet).animations["img"];
			}
...
/*class PixiAtlasButton extends PixiAtlas{
	...
	checkText(){
		...
		if(this._label != undefined){
			const label = new PIXI.Text(this._label,{fontFamily : 'Arial', fontSize: 25, fill : 0xffffff, align : 'center'});//new PIXI.Text(this._label);
...
/*class PixiButton extends PIXI.Sprite{
	constructor(parent,texture,_label){
		super();
		this.init(parent,texture,_label);
	}

	init(parent,texture,_label){
		parent.container.addChild(this);
		this.texture = parent.assets.loadImg(texture);
		this.anchor.set(0.5);

		const label = new PIXI.Text(_label,{fontFamily : 'Passion One', fontSize: 40, fill : 0xffffff, align : 'center'});//new PIXI.Text(this._label);
		label.position.set( 0 ,0);
		label.style = {'_fill':'black','_align':'right','_fontSize':20,'fontWeight':'bolder'};
